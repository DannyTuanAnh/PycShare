<?php
    ...
    if(isset($_GET['listid'])){
        ...
        $delete_all = $server->prepare("delete from user where id in ($listid)") or die ("Không thể truy vấn dữ liệu");
        if($delete_all->execute()){
            echo "<script>alert('Xóa thành công');";
            echo "window.location.href = 'user_manage.php';</script>";
        }  
        else{
    ...
    if(isset($_GET['delete'])){
        $id = $_GET['delete'];
        $id = intval($id);
        $delete = $server->prepare("delete from user where id = $id") or die ("Không thể truy vấn dữ liệu");
        if($delete->execute()){
            echo "<script>alert('Xóa thành công');";
            echo "window.location.href = 'user_manage.php';</script>";
        }  
...
<body>
    <div class="container-admin">
        ...
        <main class="content-admin">
            ...
            <section class="table-section">
                ...
                <form action="user_manage" method="get">
                    <table>
                        <thead>
                            <tr>
                                <td colspan="2">
                    ...
                    while($row = mysqli_fetch_array($result)){
                        echo "<tr>";
                        echo "<td><input type='checkbox' name = 'chon' id = 'chon' class = 'chon' value='". $row['id']. "'></td>";
                        echo "<td>". $count++ . "</td>";
                        echo "<td>". $row['user'] . "</td>";
                        echo "<td>". $row['email'] . "</td>";
                        echo "<td><form method = 'get' action = 'user_manage.php'><button name = 'delete' type = 'submit' value = '" . $row['id']."'>Xóa</button></form></td>";
                        echo "</tr>";
                    }
                ?>
    ...
    $(document).ready(function() {
        $("#delete-select").click(function() {
            ...
            window.location = "user_manage.php?listid=" + listid;
        });
    });
    </script>
</body>
